...
const uint8_t Vout_Ctrl = 10; //external voltage switch controlled by pin 10, active low
const uint8_t RE = 2; //Read pin enable for SN65HVD33, active low
const uint8_t DE = 3; //Write pin enable for SN65HVD33, active high
const uint8_t UART_EN = 7; //External UART enable, active high
...
void setup() 
{
	...
	pinMode(RE, OUTPUT); //Make read enable output
	pinMode(DE, OUTPUT);  //Make write enable output
	pinMode(UART_EN, OUTPUT); //Make UART Enable an output

	digitalWrite(Vout_Ctrl, HIGH); //Turn power on by default
	digitalWrite(RE, HIGH);  //Disable RS485 read by default (master default)
	digitalWrite(DE, HIGH);  //Enable RS485 write by default
	digitalWrite(UART_EN, HIGH); //Enable external UART by default
...
void loop() 
{
	digitalWrite(RE, LOW);  //Enable RS485 read 
	digitalWrite(DE, LOW);  //Disable RS485 write
	...
	if((Ctrl & DATAWRITE) == DATAWRITE) {  //If control bit is set, print registers to output
		digitalWrite(RE, HIGH);  //Disable RS485 read 
		digitalWrite(DE, HIGH);  //Enable RS485 write 
	...
	if((Ctrl & UART_ON) == 0 && UartActive) {  //If UART ON bit is cleared, and UART is running, turn off UART
		...
		digitalWrite(UART_EN, HIGH); //Re-enable external UART once on board serial has been turned off
	}
	...
	if((Ctrl & RESET) == RESET) {  //If reset bit if set
		...
		digitalWrite(RE, HIGH); //Leave off recieve  //DEBUG!
